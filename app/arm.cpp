...
std::vector<double> Arm2D::FK(const std::vector<double>& joint_states){
    ...
    double x = link_length_[0]*cos(joint_states[0]) + link_length_[1]*cos(joint_states[0] + joint_states[1]);
    double y = link_length_[0]*sin(joint_states[0]) + link_length_[1]*sin(joint_states[0] + joint_states[1]);
...
joint_states_vec Arm2D::IK(const std::vector<double>& eef_pose){
    ...
    double a = x*x + y*y - pow(link_length_[0],2) - pow(link_length_[1],2);
    double b = 2*link_length_[0]*link_length_[1];

    double theta_2 = acos(a/b);

    double c = link_length_[0]*sin(theta_2);
    double d = link_length_[0] + link_length_[1]*cos(theta_2);
